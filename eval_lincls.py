...
torchvision_models.__dict__.update({'wide_resnet28w2': wide_resnet28w2, 'wide_resnet28w8': wide_resnet28w8})

model_names = ['vit_small', 'vit_base', 'vit_conv_small', 'vit_conv_base', 'wide_resnet28w2', 'wide_resnet28w8'] +\
              torchvision_model_names

parser = argparse.ArgumentParser(description='PyTorch Training')
parser.add_argument('data', metavar='DIR',
                    help='path to dataset')
parser.add_argument('-a', '--arch', metavar='ARCH', default='wide_resnet28w2',
                    choices=model_names,
                    help='model architecture: ' +
                        ' | '.join(model_names) +
                        ' (default: wide_resnet28w2)')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_dataset = datasets.ImageFolder(
        traindir,
        transforms.Compose([
            transforms.RandomResizedCrop(224) if 'cifar' not in dataset_name else
    ...
    val_loader = torch.utils.data.DataLoader(
        datasets.ImageFolder(valdir, transforms.Compose([
            transforms.Resize(256 if 'cifar' not in dataset_name else 32),
            transforms.CenterCrop(224 if 'cifar' not in dataset_name else 32),
            transforms.ToTensor(),
            normalize,
        ])),
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            if epoch == args.start_epoch and os.path.isfile(args.pretrained):
